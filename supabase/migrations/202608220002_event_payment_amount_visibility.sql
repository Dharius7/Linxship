-- Show a tracking event's payment-required amount regardless of whether the
-- shipment's general billing/charges section is hidden from the public.
...
comment on function public.track_shipment(text) is
  'Returns one curated shipment timeline; per-event payment-required amounts are always shown, independent of the shipment-level billing visibility flag.';
